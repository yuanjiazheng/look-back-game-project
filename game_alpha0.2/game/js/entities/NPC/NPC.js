...
class NPC extends Entity {
    constructor(x, y, width, height, name, faction = "neutral", goldCount, maxCountGold) {
        ...
        //转化相关
        this.transformToType = null; // 转换为什么类型
    }
    ...
    if (this.target && this.target.isActive) {
        ...
        // 如果目标是金币且你是rufugee或者敌人，收集它
        if (this.target.type === "gold" && this.isInRange(this.target, this.interactionRange) && (this.name == "rufugee"||this.name == "enemy")) {
    ...
    //怪物攻击墙的行为逻辑
     attackBehaviorwall(game) {
            this.velocityX=0;
        if (this.attackCooldown <= 0) {
            this.target.takeDamage(10); // 造成10点伤害
    ...
    draw(ctx, camera) {
        ...
        if (window.DEBUG_HITBOXES) {
            ...
            ctx.font = '12px Arial';
            ctx.fillText(this.name, screenCenter.x - 20, screenCenter.y - this.detectionRange - 23);
    ...
    transform() {
        ...
        switch (this.transformToName) {
            case "civilian":
                newEntity = new Civilian(this.x, this.y, this.resourceManager);
                break;
            
            case "rufugee":
                newEntity = new Rufugee(this.x, this.y, this.resourceManager);
                break;
            // 添加更多转换类型...
            default:
                console.error(`未知的转换类型: ${this.transformToType}`);
                return null;
        }
        
        // 设置游戏引用
        newEntity.sethitbox(this.hitboxSystem);
